...
#let supervisor-name = "Invited Lecturer Vasili Nikolaev"
#let city            = "Kutaisi"
#let Month           = "June"
#let year            = "2026"

#let university-logo = "figures/images.png"
#let logo-width = 3.5cm
...
We designed a controlled experiment for five different LLMs: ChatGPT (GPT-5.5), Gemini (3.5 Flash), Claude (Sonnet 4.6), Grok (4.6 Fast), and DeepSeek-V3. Each model was given the same instructions, which included: the complete C0 grammar, a strict output format, and 51 test programs. Code snippets included standard syntax, pointers, and intentionally out-of-grammar inputs. The results were compared to the original derivation trees generated by the C0 compiler. We compared them using Selkow's algorithm and a grammar-validity.

The results showed notable inconsistencies. Claude had the highest mean structural similarity (0.864) and the lowest mean edit distance (42.4). Claude also had the highest overall mean legality (0.947), slightly ahead of Gemini (0.946). Gemini produced the most fully legal trees but struggled with pointer tasks (0.284 structural similarity). ChatGPT produced the most border-correct program derivations (23 out of 51). In contrast, DeepSeek-V3 was the worst-performing model. No model reliably rejected out-of-grammar inputs. These findings support the idea that LLMs do not internalize formal grammar rules independently.
...
One might object that producing fully-expanded derivation trees on demand is not the same thing as understanding grammar. We adopt it deliberately as an operationalization: understanding a context-free grammar cannot be observed directly, so it must be measured through a concrete, checkable task. Constructing a complete derivation tree is a suitable choice because it forces a model to apply every production rule explicitly, in order, down to the terminal symbols, which surface pattern matching alone cannot reliably reproduce. We therefore treat fully legal, reference-aligned derivation as our working definition of grammatical understanding.
...
In this chapter we will review already existing research that this study is based on. Instead of just listing the resources, we will compare the differences. How our research differs from existing ones and what problem this study aims to solve. 
...
A *derivation* is a process of applying grammar rules to turn a start symbol into a terminal string. A *derivation tree* (parse tree) is just a visualization of this process. The branches represent grammar rules, and leaves are the actual words/symbols #cite(3). For unambiguous grammars, no matter which order you apply the rules, you always end up with the same tree.
...
The specific version of C0 used in this thesis comes from Paul et al.'s *System Architecture* text book #cite(46). Previous KIU students have built the compiler for this specific C0 #cite(33) under the supervision of coauthor Wolfgang J. Paul. This version of C0 adds some extra features like typedef, struct, and pointers. they also use different symbols for pointers (') instead of the usual (`*`). This was done on purpose to avoid ambiguity in compiling. In our research we use the original grammar. 
...
Hou et al.'s systematic literature review of LLMs for software engineering found hundreds of studies. They note that very few of them focus on formal grammar correctness or parser-generated structures #cite(32). Most research is about generating or fixing code. This thesis tries to fill the gap by adding grammar-based structural evaluation.  
...
(multiplication or pointer type). construction. These probe whether models distinguish ' (dereference) from (multiplication). Note that the test programs use the compiler's ' notation, whereas the grammar supplied to the models uses ; this notation gap is addressed as a threat to validity (@chap-validity).
...
// ============================================================
//  SECTION 3.6 — IMPLEMENTATION (part of Methodology)
// ============================================================
...
+ *Validate:* Checks every node in the tree to the official C0 grammar encoded in JavaScript (`PRODUCTIONS`, `SEQ_DEF`).
...
Grammar checker runs separately. Here we see some interesting results. In some cases tree follows correct grammar rules but still gets differently structured tree. (alternative valid derivations or made-up invented paths for inputs)
...
Summary of key findings (all 51 tasks):
...
Gemini was the only model that made up grammar symbols that do not exist in C0. This happened 7 times. DeepSeek completely failed with 16,167 illegal nodes overall. This was mostly because it repeatedly failed basic rules for expressions, dominated by `noProduction` (14,006) and `unexpanded` (2,161) nodes. These mistakes weren't occasional, they were systematic, widespread failures.
...
On normal tasks, four models out of five, excluding deepSeek, scored above 0.76. Pointer task caused the biggest drop for Gemini. It failed from 0.908 to 0.284. Claude handled pointer tasks the best, scoring around 0.772. 
...
Normal tasks (green) performed well on both measures. Both pointer and out-of-grammar tasks show something interesting. They have high grammar correctness but low structural similarity. We called this *OOG paradox*. Meaning that, models can invent a tree that follows the grammar rules but still looks completely different from the correct reference. 
...
Tasks 45 - 48 (pointer block) and 49 - 51 (OOG) were consistently hard for all models. DeepSeek's row is consistently low across all tasks. It generated a massive tree with an unnecessary amount of nodes.
...
// ============================================================
//  SECTION — THREATS TO VALIDITY (part of Discussion)
// ============================================================
...
51 programs are not enough to cover full C0 grammar. Especially with only 4 pointer and 3 OOG cases. Results may not be true for other types of programs. Each task was only run once. With this approach, there is no way of knowing if the LLM would do better or worse on the second try. A single sample cannot distinguish a model's typical behavior from a lucky or unlucky draw; running each task multiple times, for example ten runs per task, would give a much better picture. We leave this to future work, as it was limited by time constraints. LLMs were instructed to never refuse a task, which pushed them to generate even when they shouldn't have. Grammar-constrained decoders #cite(25, 37) and explicit error-reporting prompts could have had different results on tasks 49-51. These results reflect only up to June 2026. Future updates may yield different results, as benchmark rankings have shifted with model generations #cite(31). For the pointer tasks, models were asked to reproduce reference structures containing a symbol absent from a supplied grammar. The low pointer scores consequently conflate genuine derivation difficulty with this notation gap and they should be read as evidence of how models behave at the boundary of supplied grammar rather than as a clean test.

=== Construct Validity

The trees are compared through mermaid code, not directly as grammar object. If the model formats the Mermaid code incorrectly it could hurt the score, even if the grammar knowledge was actually correct. Selkow distance #cite(6) penalizes alternative valid derivations equally, as documented in tree-edit surveys #cite(9). A grammatically correct but differently structured tree gets scored lower than deserved. Zhang and Shasha's metric #cite(8) would behave similarly for derivation ambiguity. The JavaScript production table may not capture every edge case of the C0 grammar. disagreements would appear between oracle and human judgment, as noted in prior grammar-constrained systems #cite(24).

=== External Validity

This research tests only one C0. These findings may not apply to other programming languages. C0 is simplified languages designed for learning purposes. Real-World languages are much bigger and more complex. Therefore, results might be different from this research.

=== Conclusion Validity

Flattening and noise removal align reference and candidate trees but alter raw structures. Sensitivity to these options was not fully researched.

Despite these limitations, the study is still reliable as it tested multiple LLM models under the same conditions. Research uses two different matrics (structure + grammar correctness) and analyzes results across different task categories. All this gave solid results of how well current AI models handle formal grammar.
...
+ Best result in terms of generating fully legal output was 10 out of 51(Gemini), and those successful results came from deriving the simplest programs, which means no model achieves reliable formal correctness. 
