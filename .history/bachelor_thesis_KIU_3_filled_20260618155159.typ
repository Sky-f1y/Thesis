...
The results were uneven across models. Claude came closest to the reference trees overall, with the highest mean structural similarity (0.864) and the lowest mean edit distance (42.4). Gemini followed the grammar most closely on standard in-grammar programs (legality 0.946), but its performance dropped sharply on pointer tasks (0.284 structural similarity). ChatGPT was the most consistent performer and produced the most border-correct program derivations (23 out of 51). DeepSeek-V3 lagged behind the others on nearly every measure. None of the models reliably refused to derive trees for out-of-grammar inputs. Taken together, these findings suggest that LLMs do not independently internalize formal grammar rules.
...
The C0 programming language evaluated in this thesis is a pedagogical dialect
whose grammar is taken from *System Architecture: An Ordinary Engineering
Discipline* #cite(46), a textbook by Petro Lutsyk, Sabine Schmaltz, Wolfgang J.
Paul, and Christoph Baumann that specifies a formal C subset for
compiler-construction exercises. At Kutaisi International University (KIU),
compiler courses adopt this dialect; co-author Wolfgang J. Paul taught the
material on site. The KIU variant retains the book's production system but uses
pointer notation (`'` for dereference, `&` for address-of, `*` for multiplication
or pointer types) that differs from mainstream C. Although it belongs to the
...
benchmarks such as HumanEval #cite(15) and MBPP #cite(16), yet those benchmarks
evaluate functional correctness of generated programs rather than step-by-step
adherence to an explicit context-free grammar #cite(12, 20). Recent surveys
confirm that most LLM-for-code research still models source as token sequences
and prioritizes downstream software engineering tasks over formal parsing
#cite(12, 32). As LLMs are adopted in education and compiler tooling, it becomes
important to determine whether they understand grammar rules or merely reproduce
statistically likely syntactic patterns #cite(27). This thesis addresses that
question through a direct, automatable evaluation of derivation tree generation.
