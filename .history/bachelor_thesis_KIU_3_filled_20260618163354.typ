...
The C0 programming language evaluated in this thesis is taken from *System Architecture: An Ordinary Engineering Discipline* #cite(46), a textbook by Petro Lutsyk, Sabine Schmaltz, Wolfgang J. Paul, and Christoph Baumann. In this book they specify a formal C subset for compiler-construction exercises. At Kutaisi International University (KIU), compiler courses adopt this grammar. Co-author Wolfgang J. Paul taught the material at the university himself. like other teaching-oriented C subsets, including Carnegie Mellon's C0 #cite(4, 5), C is reduced to simpler versiuon. This version is enough for students to learn parsing and code generation. with that said, constructing a dreivation tree for C0 grammar still remains a challenging task. It requires grammatical understanding that code generation cannot pass.

Large language models (LLMs) score well on standard coding benchmarks such as HumanEval #cite(15) and MBPP #cite(16). Those tests ask whether a program runs correctly, but not whether a model can follow an explicit context-free grammar step by step #cite(12, 20). Surveys of LLM-for-code research reach a similar conclusion: most work still treats source code as a sequence of  tokens and focuses on software engineering tasks rather than formal parsing #cite(12, 32). this becomes bigger problem as LLMs are inceasingly used in education and comiler tooling. That gap matters as LLMs move into education and compiler tooling, where it is unclear whether a model truly understands grammar rules or only reproduces syntactic patterns that look plausible #cite(27). this thesis adresses that question, by directly testing LLMs' abilities.
...
+ *RQ2:* Which large language model produces most correct derivation trees, similar to
  ones generated by the reference compiler?
+ *RQ3:* Can large language models correctly distinguish pointer syntax from
  multiplication expressions?
+ *RQ4:* How do large language models respond to syntactically absent inputs
  that are outside the C0 grammar?
+ *RQ5:* Does high tree similarity indicate better understanding of formal
  grammar rules?
