...
Large language models (LLMs) score well on standard codein benchmarks such as HumanEval #cite(15) and MBPP #cite(16). Those tests ask whether a program runs correctly, but not whether a model can follow an explicit context-free grammar step by step #cite(12, 20). Surveys of LLM-for-code research reach a similar conclusion: most work still treats source code as a sequence of  tokens and focuses on software engineering tasks rather than formal parsing #cite(12, 32). this becomes bigger problem as LLMs are inceasingly used in education and comiler tooling. That gap matters as LLMs move into education and compiler tooling, where it is unclear whether a model truly understands grammar rules or only reproduces syntactic patterns that look plausible #cite(27). this thesis adresses that question, by directly testing LLMs' abilities.
