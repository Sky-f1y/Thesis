...
The C0 dialect at the center of this thesis comes from *System Architecture: An
Ordinary Engineering Discipline* #cite(46), a textbook by Petro Lutsyk, Sabine
Schmaltz, Wolfgang J. Paul, and Christoph Baumann that defines a formal C subset
for compiler-construction exercises. At Kutaisi International University (KIU),
compiler courses use this grammar as their reference, and co-author Wolfgang J.
Paul taught the material there in person. Like other teaching-oriented C
subsets---including Carnegie Mellon's C0 #cite(4, 5)---it trims full C down to
what students need for parsing practice, but the grammar tested here is the one
from the textbook, not an ad hoc local variant. Constructing a derivation tree
for a C0 program is a strict test of grammatical understanding that approximate
code generation cannot pass.
