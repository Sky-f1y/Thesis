...
We designed a controlled experiment for five different LLMs: ChatGPT (GPT-5.5), Gemini (3.5 Flash), Claude (Sonnet 4.6), Grok (4.6 Fast), and DeepSeek-V3. Each model was given the same instructions, which included: the complete C0 grammar, a strict output format and 51 test programs, one per query. code snippets included standard syntax, pointers, and intentionally out-of-grammar inputs. The results were compared to the original derivation trees generated by the C0 compiler. Comparison was done by using Selkow's tree edit distance and a grammar-validity. Both of which were used in the `tree_diff_v2.html` file.

The results showed notable inconsistencies. Claude had the lowest mean edit distance (42.4) and highest mean structural similarity (0.864). Claude also had the highest overall mean legality (0.947), slightly ahead of Gemini (0.946). Gemini had the most number of perfectly derived trees in terms of legality, but it struggled with pointer tasks (0.284 in structural similarity). ChatGPT produced the most border-correct program derivations (23 out of 51). In contrast, DeepSeek-V3 was the worst-performing model. In addition, no model reliably rejected out-of-grammar inputs. These findings support the idea that LLMs do not internalize formal grammar rules independently.
...
(multiplication or pointer type). construction. These probe whether models distinguish ' (dereference) from (multiplication). Note that the test programs use the compiler's ' notation, whereas the grammar supplied to the models uses ; this notation gap is addressed as a threat to validity (Chapter 7).
...
Tasks 45 - 48 (pointer block) and 49 - 51 (OOG) were consistently hard for all models. DeepSeek's row is consistently low across all tasks. Task 35 (GCD while-loop) shows a near-zero score because the model kept expanding the tree. It generated a massive tree with an unnecessary amount of nodes.
...
*Pointer notation mismatch*: For the pointer tasks, models were asked to reproduce reference structures containing a symbol absent from a supplied grammar. The low pointer scores consequently conflate genuine derivation difficulty with this notation gap and they should be read as evidence of how models behave at the boundary of supplied grammar rather than as a clean test.

== Construct Validity

*Mermaid as proxy:* The trees are compared through mermaid code, not directly as grammar object. If the model formats the Mermaid code incorrectly it could hurt the score, even if the grammar knowledge was actually correct.
...
*Single language:* this research tests only one C0. These findings may not apply to other programming languages.
