...
The results showed notable inconsistencies. Claude had highest mean structural similarity (0.864) and lowest mean edit distance (42.4). while, Gemini had the highest legality (0.946) for in-grammar programs, it stuggled with pointer tasks (0.284 in structural similarity). ChatGPT was the most consistant. It produced the most border-correct program derivations (23 out of 51).On the other hand, DeepSeek-V3 was the worst performing model. In addition,no model reliably rejected out-of-grammar inputs.these finding support the idea that LLMs do not internalize formal grammar rules independently. 

#v(1.5em)
*Keywords:* large language models, derivation trees, formal grammars,
C0 programming language, tree edit distance, Selkow algorithm, syntax parsing,
Mermaid diagrams, grammar evaluation
]
...
Compiler construction rests on formal language theory: a programming language is
defined by a grammar, and a parser applies production rules to build a
derivation tree (parse tree) that records how an input string was derived from
the start symbol #cite(3). Unlike generated source code, a derivation tree
exposes every intermediate syntactic decision. Correct tree construction
therefore requires precise knowledge of non-terminal expansions, operator
precedence, lexical decomposition, and the distinction between syntactic
categories.

The C0 programming language evaluated in this thesis is a pedagogical dialect used
in compiler courses at Kutaisi International University (KIU). It follows the
tradition of teaching-oriented C subsets such as Carnegie Mellon's C0 #cite(4, 5),
but defines its own grammar with KIU-specific pointer notation (`'` for
dereference, `&` for address-of, `*` for multiplication or pointer types).
Constructing a derivation tree for a C0 program is a rigorous test of grammatical
understanding that cannot be satisfied by approximate code generation alone.
...
The C0 grammar used in this thesis is a CFG extended with lexical productions for
identifiers, digits, and character constants. Unlike English or other natural
...
projects #cite(34). The KIU variant evaluated here---grounded in the reference C0 compiler from a prior KIU bachelor's thesis #cite(33)---
extends pedagogical C0 with typedefs, structs, and pointer notation using `'`
(dereference) and `&` (address-of). This notation creates a deliberate ambiguity
...
+ The complete C0 grammar (lexical, expression, statement, and program rules) #cite(3).
