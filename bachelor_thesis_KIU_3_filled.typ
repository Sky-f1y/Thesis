...
We designed a controlled experiment for five different LLMs: ChatGPT (GPT-5.5), Gemini (3.5 Flash), Claude (Sonnet 4.6), Grok (4.6 Fast), and DeepSeek-V3. Each model was given the same instructions, which included: the complete C0 grammar, a strict output format, and 51 lines of code. code snippets included standard syntax, pointers, and intentionally out-of-grammar inputs. The results were compared to the original derivation trees generated by the C0 compiler. We Compared used Selkow's algorythm and a grammar-validity.

The results showed notable inconsistencies. Claude had the highest mean structural similarity (0.864) and the lowest mean edit distance (42.4). While Gemini had the highest legality (0.946) for in-grammar programs, it struggled with pointer tasks (0.284). ChatGPT was the most consistent. It produced the most border-correct program derivations (23 out of 51).Finally, DeepSeek-V3 was the worst-performing model. Additionally, no model rejected out-of-grammar inputs. these finding support the idea that LLMs do not internalize formal grammar rules independently.
